...
namespace OAIServer
{
    ...
    public class DatabaseMapping : FieldMapping
    {
        ...
        public override void Load(XmlNode node)
        {
            ...
            if (node.Attributes["defaultValue"] != null) this.DefaultValue = node.Attributes["defaultValue"].InnerText;
        }
        ...
        public override String GetValueSQL(DataConnection dc)
        {
            ...
            else 
            {
                ...
                {
                    val = "isnull(" + val + ",'" + DefaultValue.ToString() + "')";
                    if (this.ColumnAlias == "") val += " " + Column; //to alias itself back to its correct column name
                }
    ...
    public class SQLMaxValueMapping : DatabaseMapping
    {
        public override Object GetValue(DataConnection dc)
        {
            ...
            using (OleDbConnection cnn = new OleDbConnection(dc.DBConnStr))
            {
                ...
                if (SQL != null && SQL.Length > 0)
                {
                    cmd.CommandText = "select max(" + SQL + ") from " + dc.GetMappedTable(TableId).Name;
                }
                else
                {
    ...
    public class SQLMinValueMapping : DatabaseMapping
    {
        public override Object GetValue(DataConnection dc)
        {
            ...
            using (OleDbConnection cnn = new OleDbConnection(dc.DBConnStr))
            {
                ...
                if (SQL != null && SQL.Length > 0)
                {
                    cmd.CommandText = "select min(" + SQL + ") from " + dc.GetMappedTable(TableId).Name;
                }
                else
                {
