...
namespace OAIServer
{
    public class GetRecordRequest
    {
        ...
        private bool SetHasId(DataConnection setDC, String id)
        {
            bool hasId = false;

            if (setDC != null)
            {
                using (OleDbConnection cnn = new OleDbConnection(setDC.DBConnStr))
                {
                    cnn.Open();
                    OleDbCommand cmd = cnn.CreateCommand();

                    String sql = "select " + setDC.RootTable.PK + " from " + setDC.RootTable.Name + " where cast(" + setDC.RootTable.PK + " as nvarchar(250)) = '" + id + "'";
                    cmd.CommandText = sql;

                    object val = cmd.ExecuteScalar();

                    if (val != null && val != DBNull.Value) hasId = true;
                }
            }
        ...
        private DataSet GetResultData(String id, String set)
        {
            ...
            if (dc != null)
            {
                using (OleDbConnection cnn = new OleDbConnection(dc.DBConnStr))
                {
                    ...
                    sql += "." + dc.RootTable.PK + " as varchar(50)) = '" + id + "'";
