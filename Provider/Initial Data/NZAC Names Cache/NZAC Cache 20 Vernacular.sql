IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[NZACCache20]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[NZACCache20]
GO
...
CREATE procedure [dbo].[NZACCache20]
as
...
insert into [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzac_name.Vernacular(
...
insert into [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzac_name.VernacularConcept(
...
insert into [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzac_name.ConceptApplication(
...
select c.VernacularUseCounterPK,
	...
	--(Select top 1 conceptid from [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzac_name..Concept con where
...
from tblVernacular vn
	...
	inner join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzac_name.name n on n.nameid = vu.vernacularusenamefk
	left join tblLanguage la on vn.VernacularLanguageFk = la.LanguageCounterPK
	left join tblGeoRegion g on vn.VernacularGeoRegionFk = g.GeoRegionCounterPK
...
insert into [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzac_name.vernacularuse
	([VernacularId]
		,CreatedDate
       ,[AccordingToId]
       ,[TaxonNameId]
...
inner join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzac_name.name n on n.nameid = vu.vernacularusenamefk
inner join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzac_name.vernacular v on v.vernacularid = vu.VernacularUseVernacularFk
left join #Conrels cr on vn.VernacularGuid = cr.VernacularUseVernacularFK
left join tblVernacularArticle va on vu.VernacularUseCounterPk = va.VernacularArticleVernacularUseFK		
...
grant execute on dbo.[NZACCache20] to dbi_user
