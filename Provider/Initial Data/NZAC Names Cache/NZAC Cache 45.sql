IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[NZACCache45]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[NZACCache45]
GO
...
CREATE procedure [dbo].[NZACCache45]
as

	insert into [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzac_name.annotation
	...
	inner join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzac_name.name n on n.nameid = nn.namenotenamefk
	left join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzac_name.publication p on p.publicationid = NameNoteReferenceFk	
	...
	inner join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzac_name.name n on n.nameid = ns.NomenclaturalStatusNameFk
	left join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzac_name.publication p on p.publicationid = NomenclaturalStatusReferenceFk	
	where NomenclaturalStatusComment is not null or NomenclaturalStatusTypeText is not null

	insert into [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzac_name.annotation
	select distinct 'ProParte_' + cast(nameguid as varchar(38)),
		...
		null
	from tblname n
	inner join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzac_name.name fn on n.nameguid = fn.nameid
	...
	insert into [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzac_name.annotation
	select distinct 'NomDub_' + cast(nameguid as varchar(38)),
		cast(nameguid as varchar(38)),
		null,
		null,
		'taxonomic status',
...
grant execute on dbo.[NZACCache45] to dbi_user
