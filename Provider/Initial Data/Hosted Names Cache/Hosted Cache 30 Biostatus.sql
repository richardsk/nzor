...
delete [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.hosted_name.Biostatus;
...
with bios as (
	  select
		...
		case b.BioStatusDescription 
			...
			when 'Exotic Casual' then 'Exotic'
		...
		ROW_NUMBER() OVER(PARTITION BY n.NameGuid, geo.georegionname 
		 ...
		 inner join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].Name_Cache.hosted_name.Name na on nb.NameBiostatusNameFk
			= Na.NameId
...
from #bios b
inner join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].Name_Cache.hosted_name.taxonconcept tc on 
...
insert into [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.hosted_name.Biostatus(
