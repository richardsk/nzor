IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[FungiCache20]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[FungiCache20]
GO
...
CREATE procedure [dbo].[FungiCache20]
as
...
into #ConRels
	from tblVernacularUse vu
	 inner join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.fungi_name.name n on n.nameid = vu.vernacularusenamefk
	...
	where isnull(vu.VernacularUseSuppress,0) = 0
		...
		--and (bib.bibliographyreferencefk is null or bib.bibliographyreferencefk not in ('DE710734-3E34-4556-93A9-773CBD5A9A52')) --dictionary of fungi 10
...
insert into [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.fungi_name.Vernacular(
...
select va.VernacularArticleCounterPK as ConceptId,
	...
	where isnull(VernacularArticleIsDeleted,0) = 0
		--and (va.VernacularArticleReferenceFk is null or va.VernacularArticleReferenceFk not in ('DE710734-3E34-4556-93A9-773CBD5A9A52')) --dictionary of fungi 10


insert into [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.fungi_name.VernacularConcept(
...
insert into [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.fungi_name.ConceptApplication(
...
select distinct c.VernacularUseCounterPK,
	...
	--(Select top 1 conceptid from [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.fungi_name.Concept con where
	...
	from #ConRels cr
	 ...
	 left join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.fungi_name.TaxonConcept tc on tc.TaxonConceptId = cr.BibliographyGuid
	 left join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.fungi_name.TaxonNameUse nbc on nbc.NameID = c.VernacularUseNameFk
...
insert into [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.fungi_name.vernacularuse
	([VernacularId]
		,CreatedDate
       ,[AccordingToId]
       ,[TaxonNameId]
...
inner join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.fungi_name.name n on n.nameid = vu.vernacularusenamefk
inner join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.fungi_name.vernacular v on v.vernacularid = vu.VernacularUseVernacularFk
left join #Conrels cr on vn.VernacularGuid = cr.VernacularUseVernacularFK
...
grant execute on dbo.[FungiCache20] to dbi_user
