...
INSERT INTO [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.fungi_name.[Name](NameId, IsScientific,
...
select na.NameGuid, 1,
	...
	case when na.NameNomCode is null then 'ICBN'
		when na.NameNomCode = '' then 'ICBN'
...
where na.NameSuppress = 0 and isnull(pref.namesuppress,0) = 0 and isnull(na.NameMisapplied,0) = 0
	and (fn.FlatNameCanonical is null or (fn.FlatNameCanonical <> 'kingdomtemp' and fn.FlatNameCanonical <> 'HostList'))
...
--	from
--	spidey.FungiName_Cache.dbo.Name na 
...
--	from spidey.FungiName_Cache.dbo.Name na
...
from [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.fungi_name.Name na
...
from dbo.tblName n ) DS
	inner join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.fungi_name.Name na on DS.NameGuid = Na.NameId
...
from tblName n
inner join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.fungi_name.[Name] n2 on n2.nameid = n.NameGuid

--standardise quality codes (title case)
update [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.fungi_name.[Name] 
...
select distinct 
	...
	pn.NameCanonical,
	'Unvalidated',
	case when pn.NameNomCode is null then 'ICBN'
		when pn.NameNomCode = '' then 'ICBN'
...
where (pn.NameSuppress = 1 or pn.NameMisapplied = 1) and na.NameSuppress = 0 and isnull(na.NameMisapplied,0) = 0
	and (kfn.FlatNameCanonical is null or (kfn.FlatNameCanonical <> 'kingdomtemp' and kfn.FlatNameCanonical <> 'HostList'))
	
	
go

grant execute on dbo.[FungiCache10] to dbi_user
