IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[FungiCache45]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[FungiCache45]
GO
...
CREATE procedure [dbo].[FungiCache45]
as

	insert into [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.fungi_name.annotation
	...
	inner join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.fungi_name.name n on n.nameid = nn.namenotenamefk
	left join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.fungi_name.publication p on p.publicationid = NameNoteReferenceFk	
	...
	inner join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.fungi_name.name n on n.nameid = ns.NomenclaturalStatusNameFk
	left join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.fungi_name.publication p on p.publicationid = NomenclaturalStatusReferenceFk	
	...
	select 'ProParte_' + cast(nameguid as varchar(38)),
		...
		null
	from tblname n
	inner join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.fungi_name.name fn on n.nameguid = fn.nameid
...
grant execute on dbo.[FungiCache45] to dbi_user
