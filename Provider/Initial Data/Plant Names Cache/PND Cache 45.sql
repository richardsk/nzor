IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[PNDCache45]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[PNDCache45]
GO
...
CREATE procedure [dbo].[PNDCache45]
as

	insert into [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.plant_name.annotation
	...
	inner join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.plant_name.name n on n.nameid = nn.namenotenamefk
	left join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.plant_name.publication p on p.publicationid = NameNoteReferenceFk	
	...
	inner join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.plant_name.name n on n.nameid = ns.NomenclaturalStatusNameFk
	left join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.plant_name.publication p on p.publicationid = NomenclaturalStatusReferenceFk	
	...
	inner join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.plant_name.name pn on pn.nameid = n.nameguid
...
grant execute on dbo.[PNDCache45] to dbi_user
