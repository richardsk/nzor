...
--taxon
declare @taxon table(TaxonID uniqueidentifier, 
...
insert @taxon(TaxonID, NameID, AccordingToID, AccordingTo, ConceptID, TaxonRank, Vernacular, NomCode, TaxonRemarks, Modified, Rights, RightsHolder, AccessRights, BibCitation, InfoWithheld, DatasetID, DatasetName, Source)
...
update t
set AcceptedName = anc.NameToFull,
	AcceptedUsageID = anc.ConceptToID
from @taxon t
inner join consensus.vwConcepts anc on anc.ConceptID = t.ConceptID and anc.ConceptRelationshipTypeID = '0CA79AB3-E213-4F51-88B9-4CE01F735A1D' and anc.IsActive = 1
...
update t
set ParentUsageID = pnc.ConceptToID,
	ParentName = pnc.NameToFull 
from @taxon t
inner join consensus.vwConcepts pnc on pnc.ConceptID = t.ConceptID and pnc.ConceptRelationshipTypeID = '6A11B466-1907-446F-9229-D604579AA155' and pnc.IsActive = 1


update t
set OriginalUsageID =  c.ConceptID,
	OriginalName =  bnp.Value
from @taxon t
...
inner join consensus.vwConcepts c on c.NameID = bnp.RelatedID and c.IsActive = 1 and c.ConceptRelationshipTypeID = '6A11B466-1907-446F-9229-D604579AA155'
	

update t
set HigherClassification = h.names 
from @taxon t
cross apply (
...
update t
set TaxonomicStatus = 
	case when anc.NameToID <> t.NameID then 'synonym;' else 'accepted;' end +
	case when tnc.NameToID is not null then 'teleomorph;' else '' end +
	case when annc.NameToID is not null then 'anamorph;' else '' end
from @taxon t
left join consensus.vwConcepts anc on anc.ConceptID = t.ConceptID and anc.ConceptRelationshipTypeID = '0CA79AB3-E213-4F51-88B9-4CE01F735A1D' and anc.IsActive = 1
left join consensus.vwConcepts tnc on tnc.ConceptID = t.ConceptID and tnc.ConceptRelationshipTypeID = '1EAA046F-61A6-4F1E-B1A7-0E1D2CE78BBF' and tnc.IsActive = 1
left join consensus.vwConcepts annc on annc.ConceptID = t.ConceptID and annc.ConceptRelationshipTypeID = '0018A60A-3C1D-4EFA-B4B3-534B40ACD079' and annc.IsActive = 1
...
left join dbo.Country ct on ct.Country = gtp.Value
union
