CREATE PROCEDURE consensus.[sprTransferNZORToDarwinCore2]	
AS
	...
	insert #taxon(TaxonID, AccordingTo, TaxonRank, Vernacular, NomCode, TaxonRemarks, Modified, Rights, RightsHolder, AccessRights, BibCitation, InfoWithheld, Source)
	...
	from #taxon t
	inner join consensus.NameProperty fnp on fnp.NameID = t.TaxonID and fnp.NamePropertyTypeID = '00806321-C8BD-4518-9539-1286DA02CA7D'

	update t
	set AcceptedName = replace(c.ScientificName, ' ', ' '),
		AcceptedUsageID = toc.NameID
	from #taxon t
	inner join consensus.Concept cc on cc.NameID = t.TaxonID
	inner join consensus.ConceptRelationship anc on anc.FromConceptID = cc.ConceptID and anc.ConceptRelationshipTypeID = '0CA79AB3-E213-4F51-88B9-4CE01F735A1D' and anc.IsActive = 1
	inner join consensus.Concept toc on toc.ConceptID = anc.ToConceptID
	inner join #taxon c on c.TaxonID = toc.NameID
	...
	from #taxon t
	inner join consensus.NameProperty pinp on pinp.NameID = t.TaxonID and pinp.NamePropertyTypeID = 'DEDC63F0-FB2A-420B-9932-786B4347DA45'

	update t
	set ParentUsageID = case when isnull(pt.AcceptedUsageID, pt.TaxonID) = pt.TaxonID then pt.TaxonID else pt.AcceptedUsageID end,
		ParentName = case when isnull(pt.AcceptedUsageID, pt.TaxonID) = pt.TaxonID then replace(pt.ScientificName, ' ', ' ') else replace(pt.AcceptedName, ' ', ' ') end
	from #taxon t
	inner join consensus.Concept cc on cc.NameID = t.TaxonID
	inner join consensus.ConceptRelationship pnc on pnc.FromConceptID = cc.ConceptID and pnc.ConceptRelationshipTypeID = '6A11B466-1907-446F-9229-D604579AA155' and pnc.IsActive = 1
	inner join consensus.Concept toc on toc.ConceptID = pnc.ToConceptID
	inner join #taxon pt on pt.TaxonID = toc.NameID


	update t
	set OriginalUsageID =  fnp.NameID,
		OriginalName =  fnp.Value
	from #taxon t
	inner join consensus.NameProperty bnp on bnp.NameID = t.TaxonID and bnp.NamePropertyTypeID = 'F496FBCC-8DA6-4CA1-9884-11BD9B5DF63B'	
	inner join consensus.NameProperty fnp on fnp.NameID = bnp.RelatedID and fnp.NamePropertyTypeID = '00806321-C8BD-4518-9539-1286DA02CA7D'	
	inner join #taxon ct on ct.TaxonID = fnp.NameID
	...
	cross apply (
		select (SELECT hn.CanonicalName  + ';' AS [text()] from (select distinct top 100 canonicalname, sortorder
								...
								where sn.SeedNameID = t.TaxonID and CanonicalName <> 'ROOT' and sn.NameID <> t.TaxonID
	...
	from #taxon t
	inner join consensus.StackedName kn on kn.SeedNameID = t.TaxonID and kn.RankName = 'kingdom'

	update t
	set Phylum = pn.CanonicalName
	from #taxon t
	inner join consensus.StackedName pn on pn.SeedNameID = t.TaxonID and pn.RankName = 'phylum'

	update t
	set Class = cn.CanonicalName
	from #taxon t
	inner join consensus.StackedName cn on cn.SeedNameID = t.TaxonID and cn.RankName = 'class'

	update t
	set [Order] = orn.CanonicalName
	from #taxon t
	inner join consensus.StackedName orn on orn.SeedNameID = t.TaxonID and orn.RankName = 'order'

	update t
	set Family = fn.CanonicalName
	from #taxon t
	inner join consensus.StackedName fn on fn.SeedNameID = t.TaxonID and fn.RankName = 'family'

	update t
	set Genus = gn.CanonicalName
	from #taxon t
	inner join consensus.StackedName gn on gn.SeedNameID = t.TaxonID and gn.RankName = 'genus'

	update t
	set SubGenus = sgn.CanonicalName
	from #taxon t
	inner join consensus.StackedName sgn on sgn.SeedNameID = t.TaxonID and sgn.RankName = 'subgenus'

	update t
	set SpecificEpithet = spn.CanonicalName
	from #taxon t
	inner join consensus.StackedName spn on spn.SeedNameID = t.TaxonID and spn.RankName = 'species'

	update t
	set InfraspecificEpithet = isn.CanonicalName
	from #taxon t
	inner join consensus.StackedName isn on isn.SeedNameID = t.TaxonID and isn.SortOrder > 4200 and isn.Depth = 1

	update t
	set VerbatimRank = rnp.Value
	from #taxon t
	inner join consensus.NameProperty rnp on rnp.NameID = t.TaxonID and rnp.NamePropertyTypeID = 'A1D57520-3D64-4F7D-97C8-69B449AFA280'

	update t
	set Authors = anp.Value
	from #taxon t
	inner join consensus.NameProperty anp on anp.NameID = t.TaxonID and anp.NamePropertyTypeID = '006D86A8-08A5-4C1A-BC08-C07B0225E01B'

	update #taxon
	set TaxonomicStatus = case when AcceptedUsageID <> TaxonID then 'synonym' else 'accepted' end 
	...
	from #taxon t
	inner join consensus.Concept cc on cc.NameID = t.TaxonID
	inner join consensus.ConceptRelationship tnc on tnc.FromConceptID = cc.ConceptID and tnc.ConceptRelationshipTypeID = '1EAA046F-61A6-4F1E-B1A7-0E1D2CE78BBF' and tnc.IsActive = 1
	...
	from #taxon t
	inner join consensus.Concept cc on cc.NameID = t.TaxonID
	inner join consensus.ConceptRelationship annc on annc.FromConceptID = cc.ConceptID and annc.ConceptRelationshipTypeID = '0018A60A-3C1D-4EFA-B4B3-534B40ACD079' and annc.IsActive = 1

	update t
	set NomenclaturalStatus = nsnp.Value
	from #taxon t
	inner join consensus.NameProperty nsnp on nsnp.NameID = t.TaxonID and nsnp.NamePropertyTypeID = '65F94532-7687-4958-B5B8-71F54866BEAD'
	...
	inner join consensus.Concept cc on cc.ConceptID = ca.ToConceptID
	inner join darwincore.Taxon t on t.TaxonID = cc.NameID
	where n.NameClassID = '05BCC19C-27E8-492C-8ADD-EC5F73325BC5' 


	--literature
	insert into darwincore.Literature
	select distinct c.NameID,
		...
		'' as Type
	from consensus.Concept c
	inner join darwincore.Taxon t on t.TaxonID = c.NameID
	inner join consensus.Name n on n.NameID = c.NameID
	...
	inner join consensus.TaxonProperty tp on tp.NameID = t.TaxonID
	...
	inner join consensus.Concept cc on cc.NameID = c.TaxonID
	inner join consensus.TaxonProperty tp on tp.ConceptID = cc.ConceptID
	inner join consensus.TaxonPropertyValue gtp on gtp.TaxonPropertyID = tp.TaxonPropertyID and gtp.TaxonPropertyTypeID = 'BABCDC8B-E40B-43A8-B6F6-88C97B9197A0'
	left join consensus.TaxonPropertyValue octp on octp.TaxonPropertyID = tp.TaxonPropertyID and octp.TaxonPropertyTypeID = '9BB63B14-0208-4070-A575-94F90DFD47B0'
	left join consensus.TaxonPropertyValue etp on etp.TaxonPropertyID = tp.TaxonPropertyID and etp.TaxonPropertyTypeID = 'D955AD6E-4678-4AC9-B752-6A94F1C07080'
