...
(select 'NZAC' "@id", --DataSource
    (Select Disclaimer,
		Attribution,
		Licensing
			from NZAC_name.Metadata
  ...
  (select (select p.PublicationID "@id",
	...
	(select 'of publication' "@type",
		...
		from NZAC_name.Publication where PublicationID = p.PublicationID
	...
	(select		
	case when not exists(select * from NZAC_name.publicationtitle where publicationid = p.publicationid) then	  
		(select
		  1 "@level",
		  'full' "@type",
		  coalesce(Citation,'') [text()]
		  from NZAC_name.Publication where
		...
		(select
		  [level] "@level",
			[type] "@type",
			coalesce(Title,'') [text()]
			from NZAC_name.PublicationTitle where
				PublicationId = p.publicationid
			for xml path('Title'), type)
	end
...
(select -- Name element
	...
	case when PublishedInId is null then 
		(select coalesce(PublishedIn,'') from NZAC_name.Name where NameId = n1.nameid for xml path('PublishedIn'), type)
	else 
		(select PublishedInId "@ref", coalesce(PublishedIn,'') [text()] from NZAC_name.Name where nameid = n1.nameid
			for xml Path('PublishedIn'),  type)
	end,
	coalesce([Year],'') 'Year',
	coalesce(MicroReference,'') 'MicroReference',
	case when TypeName is null then '' else
	  (select TypeNameId "@ref", TypeName [text()] from NZAC_name.Name where nameid = n1.nameid
		for xml path('TypeName'), type)
	end,
	ProtologueOrthography as Orthography,
	case when BasionymId is null then '' else
	  (select BasionymId "@ref", Basionym [text()] from NZAC_name.Name where nameid = n1.nameid
		for xml path('Basionym'), type)
	end,
	case when LaterHomonymOfId is null then '' else
	  (select LaterHomonymOfId "@ref", LaterHomonymOf [text()]
		from NZAC_name.Name where nameid = n1.nameid
...
(select vernacularid "@id",
		 ...
		 (select PublishedInId "@ref", PublishedIn [text()]
			from NZAC_name.Vernacular where VernacularId = v1.vernacularId
			for xml path('PublishedIn'), type),
		 [Language],
		 Country
		 from NZAC_name.Vernacular v1
...
(select
	...
	(select NameId "@ref", 
		Name [text()]
		from NZAC_name.taxonconcept where taxonconceptid = c1.taxonconceptid
		for xml path('TaxonName'), type),
	...
	(select vernacularconceptid "@id",
		...
		(select NameId "@ref", 			
			Name [text()]
			from NZAC_name.vernacularconcept where vernacularconceptid = cv1.vernacularconceptid
			for xml path('VernacularName'), type),
		(select AccordingToId "@ref", 
			AccordingTo [text()]
			from NZAC_name.vernacularconcept where vernacularconceptid = cv1.vernacularconceptid
		...
		(select
			...
			(select GeographicSchema "@geographicSchema",
				GeoRegion [text()]
				from NZAC_name.ConceptApplication where RowId = ca1.RowId 					
				for XML path('GeoRegion'), type)
			from
			NZAC_name.ConceptApplication ca1 where FromConceptId  = cv1.vernacularconceptid
				and isnull(ToConceptId,ToNameId) is not null
			for XML Path('Application'), type)
		for xml Path('Applications'), type)
	from NZAC_name.VernacularConcept cv1
	for xml path ('VernacularConcept'), type),
	(select 
		(select createdDate "@createdDate",
				nc1.ModifiedDate "@modifiedDate",
			(select NameId "@ref"
				from NZAC_name.taxonnameuse where taxonnameuseid = nc1.taxonnameuseid
		...
		from NZAC_name.taxonnameuse nc1	
		for xml path ('TaxonNameUse'), type),
		--vernacular uses
		(select createdDate "@createdDate",
				vu1.ModifiedDate "@modifiedDate",
			(select VernacularId "@ref"
				from NZAC_name.vernacularuse where vernacularuseid = vu1.vernacularuseid
				for xml path('VernacularName'), type),
	...
	(select --Taxon Properties
		...
		(select 
			...
			(select NameId "@nameRef",
				...
				from NZAC_name.Biostatus where BiostatusID = b1.BiostatusID
	...
	case when exists(select * from nzac_name.annotation) then
		(select -- Annotations
			...
			(select NameId "@nameRef",
				ConceptID "@conceptRef"
				from nzac_name.Annotation where AnnotationID = a1.AnnotationID
				for XML path('Taxon'), type),
			Annotation "AnnotationText",
			(select AccordingToId "@ref"
				from nzac_name.Annotation where AnnotationID = a1.AnnotationID
				for XML path('AccordingTo'), type)
			from nzac_name.annotation a1
			for xml path('Annotation'), type)
		for xml path('Annotations'), type)
	else '' end

  from NZAC_name.Metadata for xml Path ('DataSource'), type)
