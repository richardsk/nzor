...
namespace NZOR.Matching.Batch.Helpers
{
    static class SearchHelper
    {
        ...
        public static string BuildSearchString(string query, bool useWildcard)
        {
                ...
                switch (strToken.ToUpper())
                {
                    ...
                    case QUOTE:
                        ...
                        if (state == ETokenState.InQuote)
                        {
                            ...
                            if (useWildcard && !searchText.EndsWith(ASTERISK))
                            {
                                searchText += ASTERISK;
                            }
                        }
                    ...
                    default:
                        ...
                        else
                        {
                            ...
                            else
                            {
                                if (useWildcard && !searchText.EndsWith(ASTERISK))
                                {
                                    searchText += SPACE + strToken + ASTERISK;
                                }
                                else
                                {
        ...
        private static string CleanSearchString(string searchString)
        {
            if (searchString != string.Empty)
            {
                ...
                //Temporary lucene work-arounds
                if (searchString.IndexOf(" not.") != -1)
                {
                    searchString = searchString.Replace(" not.", " \"not.\"");
                }
                searchString = searchString.Replace(". ", " ");
