CREATE PROCEDURE dwc_hosted.[sprTransferNZORToDwC_Hosted]	
AS

	IF  EXISTS (select * FROM sys.foreign_keys WHERE name = N'frkDistributionTaxon')
	ALTER TABLE [dwc_hosted].[Distribution] DROP CONSTRAINT [frkDistributionTaxon]

	IF  EXISTS (select * FROM sys.foreign_keys WHERE name = N'frkLiteratureTaxon')
	ALTER TABLE [dwc_hosted].[Literature] DROP CONSTRAINT [frkLiteratureTaxon]

	IF  EXISTS (select * FROM sys.foreign_keys WHERE name = N'frkVernacularTaxon')
	ALTER TABLE [dwc_hosted].[Vernacular] DROP CONSTRAINT [frkVernacularTaxon]



	truncate table dwc_hosted.Literature
	truncate table dwc_hosted.Vernacular
	truncate table dwc_hosted.Distribution
	truncate table dwc_hosted.Taxon



	ALTER TABLE [dwc_hosted].[Vernacular]  WITH CHECK ADD  CONSTRAINT [frkVernacularTaxon] FOREIGN KEY([TaxonID])
	REFERENCES [dwc_hosted].[Taxon] ([TaxonID])

	ALTER TABLE [dwc_hosted].[Vernacular] CHECK CONSTRAINT [frkVernacularTaxon]

	ALTER TABLE [dwc_hosted].[Literature]  WITH CHECK ADD  CONSTRAINT [frkLiteratureTaxon] FOREIGN KEY([TaxonID])
	REFERENCES [dwc_hosted].[Taxon] ([TaxonID])

	ALTER TABLE [dwc_hosted].[Literature] CHECK CONSTRAINT [frkLiteratureTaxon]

	ALTER TABLE [dwc_hosted].[Distribution]  WITH CHECK ADD  CONSTRAINT [frkDistributionTaxon] FOREIGN KEY([TaxonID])
	REFERENCES [dwc_hosted].[Taxon] ([TaxonID])

	ALTER TABLE [dwc_hosted].[Distribution] CHECK CONSTRAINT [frkDistributionTaxon]
	...
	select distinct c.ProviderRecordID,
		...
		n.ModifiedDate,
		'' as Rights,
		'NZOR Hosted' as RightsHolder,
		'' as AccessRights,
		'' as BibliographicCitation,
		'' as InformationWitheld,
		'' as Source,
		'New Zealand Organisms Register Hosted Names',
		c.AddedDate,
	...
	where n.DataSourceID = '144D59E3-C85D-4173-B25A-41EE6008B6C6' and n.NameClassID = 'A5233111-61A0-4AE6-9C2B-5E8E71F1473A'
	...
	--vernaculars
	insert into dwc_hosted.Vernacular
	...
	inner join dwc_hosted.Taxon t on t.TaxonID = cto.ProviderRecordID
	where n.NameClassID = '05BCC19C-27E8-492C-8ADD-EC5F73325BC5' 


	--literature
	insert into dwc_hosted.Literature
	select distinct c.ProviderRecordID,
		...
		'NZOR Hosted' as Rights, 
		'' as TaxonRemarks,
		'' as Type
	from provider.Concept c
	inner join dwc_hosted.Taxon t on t.TaxonID = c.ProviderRecordID
	inner join provider.Name n on n.NameID = c.NameID
	...
	INSERT INTO [dwc_hosted].[Distribution]
			   ([TaxonID]
			   ,[LocationID]
			   ,[Locality]
			   ,[CountryCode]
	...
	select distinct t.TaxonID,
		...
		null as StartDay,
		null as EndDay,
		'http://hosted2.landcareresearch.co.nz/default.aspx?NameId=' + c.ProviderNameID as Source,
		null as Remarks
	from dwc_hosted.Taxon t
	...
	select distinct c.TaxonID,
		...
		null as StartDay,
		null as EndDay,
		'http://hosted2.landcareresearch.co.nz/default.aspx?NameId=' + pc.ProviderNameID as Source,
		null as Remarks
	from dwc_hosted.Taxon c
	...
	insert dwc_hosted.ResourceRelationship
