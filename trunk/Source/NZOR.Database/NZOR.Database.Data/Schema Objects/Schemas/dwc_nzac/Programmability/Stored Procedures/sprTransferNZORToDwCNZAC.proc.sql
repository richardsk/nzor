CREATE PROCEDURE dwc_NZAC.[sprTransferNZORToDwC_NZAC2]	
AS

	IF  EXISTS (select * FROM sys.foreign_keys WHERE name = N'frkDistributionTaxon')
	ALTER TABLE [dwc_NZAC].[Distribution] DROP CONSTRAINT [frkDistributionTaxon]

	IF  EXISTS (select * FROM sys.foreign_keys WHERE name = N'frkLiteratureTaxon')
	ALTER TABLE [dwc_NZAC].[Literature] DROP CONSTRAINT [frkLiteratureTaxon]

	IF  EXISTS (select * FROM sys.foreign_keys WHERE name = N'frkVernacularTaxon')
	ALTER TABLE [dwc_NZAC].[Vernacular] DROP CONSTRAINT [frkVernacularTaxon]



	truncate table dwc_NZAC.Literature
	truncate table dwc_NZAC.Vernacular
	truncate table dwc_NZAC.Distribution
	truncate table dwc_NZAC.Taxon



	ALTER TABLE [dwc_NZAC].[Vernacular]  WITH CHECK ADD  CONSTRAINT [frkVernacularTaxon] FOREIGN KEY([TaxonID])
	REFERENCES [dwc_NZAC].[Taxon] ([TaxonID])

	ALTER TABLE [dwc_NZAC].[Vernacular] CHECK CONSTRAINT [frkVernacularTaxon]

	ALTER TABLE [dwc_NZAC].[Literature]  WITH CHECK ADD  CONSTRAINT [frkLiteratureTaxon] FOREIGN KEY([TaxonID])
	REFERENCES [dwc_NZAC].[Taxon] ([TaxonID])

	ALTER TABLE [dwc_NZAC].[Literature] CHECK CONSTRAINT [frkLiteratureTaxon]

	ALTER TABLE [dwc_NZAC].[Distribution]  WITH CHECK ADD  CONSTRAINT [frkDistributionTaxon] FOREIGN KEY([TaxonID])
	REFERENCES [dwc_NZAC].[Taxon] ([TaxonID])

	ALTER TABLE [dwc_NZAC].[Distribution] CHECK CONSTRAINT [frkDistributionTaxon]
	...
	select distinct n.ProviderRecordID,
		n.NameID,
		replace(n.FullName, ' ', ' '),		
		'New Zealand Arthropod Collection Names',
		...
		n.ModifiedDate,
		'' as Rights,
		'New Zealand Arthropod Collection Names' as RightsHolder,
		'' as AccessRights,
	...
	where n.DataSourceID = 'DC793129-7C07-4B4C-B496-B0D9AAE4620F' and n.NameClassID = 'A5233111-61A0-4AE6-9C2B-5E8E71F1473A'
	...
	--vernaculars
	insert into dwc_NZAC.Vernacular
	...
	inner join dwc_NZAC.Taxon t on t.TaxonID = nto.ProviderRecordID collate Latin1_General_CI_AS
	where n.NameClassID = '05BCC19C-27E8-492C-8ADD-EC5F73325BC5' 


	--literature
	insert into dwc_NZAC.Literature
	select distinct n.ProviderRecordID,
		...
		'New Zealand Arthropod Collection Names' as Rights, 
		'' as TaxonRemarks,
		'' as Type
	from provider.Name n
	inner join dwc_NZAC.Taxon t on t.TaxonID = n.ProviderRecordID collate Latin1_General_CI_AS
	inner join provider.Concept c on c.NameID = n.NameID
	...
	INSERT INTO [dwc_NZAC].[Distribution]
			   ([TaxonID]
			   ,[LocationID]
			   ,[Locality]
			   ,[CountryCode]
	...
	insert dwc_NZAC.ResourceRelationship
