CREATE PROCEDURE dwc_nzfungi.[sprTransferNZORToDwC_nzfungi]	
AS

	IF  EXISTS (select * FROM sys.foreign_keys WHERE name = N'frkDistributionTaxon')
	ALTER TABLE [dwc_nzfungi].[Distribution] DROP CONSTRAINT [frkDistributionTaxon]

	IF  EXISTS (select * FROM sys.foreign_keys WHERE name = N'frkLiteratureTaxon')
	ALTER TABLE [dwc_nzfungi].[Literature] DROP CONSTRAINT [frkLiteratureTaxon]

	IF  EXISTS (select * FROM sys.foreign_keys WHERE name = N'frkVernacularTaxon')
	ALTER TABLE [dwc_nzfungi].[Vernacular] DROP CONSTRAINT [frkVernacularTaxon]



	truncate table dwc_nzfungi.Literature
	truncate table dwc_nzfungi.Vernacular
	truncate table dwc_nzfungi.Distribution
	truncate table dwc_nzfungi.Taxon



	ALTER TABLE [dwc_nzfungi].[Vernacular]  WITH CHECK ADD  CONSTRAINT [frkVernacularTaxon] FOREIGN KEY([TaxonID])
	REFERENCES [dwc_nzfungi].[Taxon] ([TaxonID])

	ALTER TABLE [dwc_nzfungi].[Vernacular] CHECK CONSTRAINT [frkVernacularTaxon]

	ALTER TABLE [dwc_nzfungi].[Literature]  WITH CHECK ADD  CONSTRAINT [frkLiteratureTaxon] FOREIGN KEY([TaxonID])
	REFERENCES [dwc_nzfungi].[Taxon] ([TaxonID])

	ALTER TABLE [dwc_nzfungi].[Literature] CHECK CONSTRAINT [frkLiteratureTaxon]

	ALTER TABLE [dwc_nzfungi].[Distribution]  WITH CHECK ADD  CONSTRAINT [frkDistributionTaxon] FOREIGN KEY([TaxonID])
	REFERENCES [dwc_nzfungi].[Taxon] ([TaxonID])

	ALTER TABLE [dwc_nzfungi].[Distribution] CHECK CONSTRAINT [frkDistributionTaxon]
	...
	select distinct c.ProviderRecordID,
		...
		n.ModifiedDate,
		'' as Rights,
		'NZFungi' as RightsHolder,
		'' as AccessRights,
		'' as BibliographicCitation,
		'' as InformationWitheld,
		'http://nzfungi2.landcareresearch.co.nz/default.aspx?NameId=' + cast(n.ProviderRecordID as varchar(38)) as Source,
	...
	where n.DataSourceID = '175D49CD-0785-4008-BB56-04DF3E46DE13' and n.NameClassID = 'A5233111-61A0-4AE6-9C2B-5E8E71F1473A'
	...
	--vernaculars
	insert into dwc_nzfungi.Vernacular
	...
	inner join dwc_nzfungi.Taxon t on t.TaxonID = cto.ProviderRecordID
	where n.NameClassID = '05BCC19C-27E8-492C-8ADD-EC5F73325BC5' 


	--literature
	insert into dwc_nzfungi.Literature
	select distinct c.ProviderRecordID,
		...
		'' as Type
	from provider.Concept c
	inner join dwc_nzfungi.Taxon t on t.TaxonID = c.ProviderRecordID
	inner join provider.Name n on n.NameID = c.NameID
	...
	INSERT INTO [dwc_nzfungi].[Distribution]
			   ([TaxonID]
			   ,[LocationID]
			   ,[Locality]
			   ,[CountryCode]
	...
	select distinct t.TaxonID,
		...
		null as StartDay,
		null as EndDay,
		'http://nzfungi2.landcareresearch.co.nz/default.aspx?NameId=' + c.ProviderNameID as Source,
		null as Remarks
	from dwc_nzfungi.Taxon t
	...
	select distinct c.TaxonID,
		...
		null as StartDay,
		null as EndDay,
		'http://nzfungi2.landcareresearch.co.nz/default.aspx?NameId=' + pc.ProviderNameID as Source,
		null as Remarks
	from dwc_nzfungi.Taxon c
	...
	insert dwc_nzfungi.ResourceRelationship
