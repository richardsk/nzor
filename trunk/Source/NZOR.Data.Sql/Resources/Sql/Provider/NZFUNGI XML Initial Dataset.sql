...
(select 'NZFUNGI' "@id", --DataSource
    (Select Disclaimer,
		Attribution,
		Licensing
			from fungi_name.Metadata
  ...
  (select (select p.PublicationID "@id",
	...
	(select 'of publication' "@type",
		...
		from fungi_name.Publication where PublicationID = p.PublicationID
	...
	(select		
	case when not exists(select * from fungi_name.publicationtitle where publicationid = p.publicationid) then	  
		(select
		  1 "@level",
		  'full' "@type",
		  coalesce(Citation,'') [text()]
		  from fungi_name.Publication where
		...
		(select
		  [level] "@level",
			[type] "@type",
			coalesce(Title,'') [text()]
			from fungi_name.PublicationTitle where
				PublicationId = p.publicationid
			for xml path('Title'), type)
	end
...
(select -- Name element
	...
	case when PublishedInId is null then 
		(select coalesce(PublishedIn,'') from fungi_name.Name where NameId = n1.nameid for xml path('PublishedIn'), type)
	else 
		(select PublishedInId "@ref", coalesce(PublishedIn,'') [text()] from fungi_name.Name where nameid = n1.nameid
			for xml Path('PublishedIn'),  type)
	end,
	coalesce([Year],'') 'Year',
	coalesce(MicroReference,'') 'MicroReference',
	case when TypeName is null then '' else
	  (select TypeNameId "@ref", TypeName [text()] from fungi_name.Name where nameid = n1.nameid
		for xml path('TypeName'), type)
	end,
	ProtologueOrthography as Orthography,
	case when BasionymId is null then '' else
	  (select BasionymId "@ref", Basionym [text()] from fungi_name.Name where nameid = n1.nameid
		for xml path('Basionym'), type)
	end,
	case when LaterHomonymOfId is null then '' else
	  (select LaterHomonymOfId "@ref", LaterHomonymOf [text()]
		from fungi_name.Name where nameid = n1.nameid
...
(select vernacularid "@id",
		 ...
		 (select PublishedInId "@ref", PublishedIn [text()]
			from fungi_name.Vernacular where VernacularId = vn1.vernacularId
			for xml path('PublishedIn'), type),
		 [Language],
		 Country
		 from fungi_name.Vernacular vn1
...
(select
	...
	(select NameId "@ref", 
		Name [text()]
		from fungi_name.taxonconcept where taxonconceptid = c1.taxonconceptid
		for xml path('TaxonName'), type),
	...
	case when ParentConceptID is null then '' else 
	 ...
	 (Select ToConceptId "@ref",
		...
		from fungi_name.ConceptRelationship
		where FromConceptId = c1.taxonConceptId
		for XML path ('Relationship'), type)
		
		for xml path('Relationships'), type)
	from fungi_name.taxonConcept c1
	for xml path ('TaxonConcept'), type),
	case when exists(select cv1.vernacularconceptid from fungi_name.VernacularConcept cv1) then 
		(select vernacularconceptid "@id",
				createdDate "@createdDate",
				cv1.ModifiedDate "@modifiedDate",
			(select NameId "@ref", 			
				Name [text()]
				from fungi_name.vernacularconcept where vernacularconceptid = cv1.vernacularconceptid
				for xml path('VernacularName'), type),
			(select AccordingToId "@ref", 
				AccordingTo [text()]
				from fungi_name.vernacularconcept where vernacularconceptid = cv1.vernacularconceptid
			...
			(select
				...
				(select GeographicSchema "@geographicSchema",
					GeoRegion [text()]
					from fungi_name.ConceptApplication where RowId = ca1.RowId 
					
					for XML path('GeoRegion'), type)
				from
				fungi_name.ConceptApplication ca1 where FromConceptId  = cv1.vernacularconceptid
					and isnull(ToConceptId,ToNameId) is not null
				for XML Path('Application'), type)
			for xml Path('Applications'), type)
		from fungi_name.VernacularConcept cv1
		where exists(select conceptapplicationid from fungi_name.ConceptApplication where fromconceptid = cv1.vernacularconceptid)
	...
	(select 
		(select createdDate "@createdDate",
			...
			(select NameId "@ref"
				from fungi_name.taxonnameuse where taxonnameuseid = nc1.taxonnameuseid
		...
		from fungi_name.taxonnameuse nc1	
		for xml path ('TaxonNameUse'), type),
		--vernacular uses
		(select createdDate "@createdDate",
				vu1.ModifiedDate "@modifiedDate",
			(select VernacularId "@ref"
				from fungi_name.vernacularuse where vernacularuseid = vu1.vernacularuseid
				for xml path('VernacularName'), type),
	...
	(select --Taxon Properties
		...
		(select 
			...
			(select NameId "@nameRef",
				...
				from fungi_name.Biostatus where BiostatusID = b1.BiostatusID
	...
	case when exists(select * from fungi_name.annotation) then
		(select -- Annotations
			...
			(select NameId "@nameRef",
				ConceptID "@conceptRef"
				from fungi_name.Annotation where AnnotationID = a1.AnnotationID
				for XML path('Taxon'), type),
			Annotation "AnnotationText",
			(select AccordingToId "@ref"
				from fungi_name.Annotation where AnnotationID = a1.AnnotationID
				for XML path('AccordingTo'), type)
			from fungi_name.annotation a1
			for xml path('Annotation'), type)
		for xml path('Annotations'), type)
	else '' end

  from fungi_name.Metadata for xml Path ('DataSource'), type)
