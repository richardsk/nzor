...
(select 'NZFLORA' "@id", --DataSource
    (Select Disclaimer,
		Attribution,
		Licensing
			from plant_name.Metadata
  ...
  (select (select p.PublicationID "@id",
	...
	(select 'of publication' "@type",
		...
		from plant_name.Publication where PublicationID = p.PublicationID
	...
	(select		
	case when not exists(select * from plant_name.publicationtitle where publicationid = p.publicationid) then	  
		(select
		  1 "@level",
		  'full' "@type",
		  coalesce(Citation,'') [text()]
		  from plant_name.Publication where
		...
		(select
		  [level] "@level",
			[type] "@type",
			coalesce(Title,'') [text()]
			from plant_name.PublicationTitle where
				PublicationId = p.publicationid
			for xml path('Title'), type)
	end
...
(select -- Name element
	...
	case when PublishedInId is null then 
		(select coalesce(PublishedIn,'') from plant_name.Name where NameId = n1.nameid for xml path('PublishedIn'), type)
	else 
		(select PublishedInId "@ref", coalesce(PublishedIn,'') [text()] from plant_name.Name where nameid = n1.nameid
			for xml Path('PublishedIn'),  type)
	end,
	coalesce([Year],'') 'Year',
	coalesce(MicroReference,'') 'MicroReference',
	case when TypeName is null then '' else
	  (select TypeNameId "@ref", TypeName [text()] from plant_name.Name where nameid = n1.nameid
		for xml path('TypeName'), type)
	end,
	ProtologueOrthography as Orthography,
	case when BasionymId is null then '' else
	  (select BasionymId "@ref", Basionym [text()] from plant_name.Name where nameid = n1.nameid
		for xml path('Basionym'), type)
	end,
	case when LaterHomonymOfId is null then '' else
	  (select LaterHomonymOfId "@ref", LaterHomonymOf [text()]
		from plant_name.Name where nameid = n1.nameid
	...
	from plant_name.Name n1
		for xml Path('TaxonName'), type),
-- start of vernacular records
(select vernacularid "@id",
		 createdDate "@createdDate",
		 vn1.modifiedDate "@modifiedDate",
		 NameFull as FullName,
		 (select PublishedInId "@ref", PublishedIn [text()]
			from plant_name.Vernacular where VernacularId = vn1.vernacularId
			for xml path('PublishedIn'), type),
		 [Language],
		 Country
		 from plant_name.Vernacular vn1
...
(select
	...
	(select NameId "@ref", 
		Name [text()]
		from plant_name.taxonconcept where taxonconceptid = c1.taxonconceptid
		for xml path('TaxonName'), type),
	...
	case when ParentConceptID is null then '' else 
	 ...
	 (Select ToConceptId "@ref",
		...
		from plant_name.ConceptRelationship
		where FromConceptId = c1.taxonConceptId
		for XML path ('Relationship'), type)
		
		for xml path('Relationships'), type)
	from plant_name.taxonConcept c1
	for xml path ('TaxonConcept'), type),
	case when exists(select cv1.vernacularconceptid from plant_name.VernacularConcept cv1) then 
		(select vernacularconceptid "@id",
				createdDate "@createdDate",
				cv1.ModifiedDate "@modifiedDate",
			(select NameId "@ref", 			
				Name [text()]
				from plant_name.vernacularconcept where vernacularconceptid = cv1.vernacularconceptid
				for xml path('VernacularName'), type),
			(select AccordingToId "@ref", 
				AccordingTo [text()]
				from plant_name.vernacularconcept where vernacularconceptid = cv1.vernacularconceptid
			...
			(select
				...
				(select GeographicSchema "@geographicSchema",
					GeoRegion [text()]
					from plant_name.ConceptApplication where RowId = ca1.RowId 
					
					for XML path('GeoRegion'), type)
				from
				plant_name.ConceptApplication ca1 where FromConceptId  = cv1.vernacularconceptid
					and isnull(ToConceptId,ToNameId) is not null
				for XML Path('Application'), type)
			for xml Path('Applications'), type)
		from plant_name.VernacularConcept cv1
		where exists(select conceptapplicationid from plant_name.ConceptApplication where fromconceptid = cv1.vernacularconceptid)
	...
	(select 
		(select createdDate "@createdDate",
			...
			(select NameId "@ref"
				from plant_name.taxonnameuse where taxonnameuseid = nc1.taxonnameuseid
		...
		from plant_name.taxonnameuse nc1	
		for xml path ('TaxonNameUse'), type),
		--vernacular uses
		(select createdDate "@createdDate",
				vu1.ModifiedDate "@modifiedDate",
			(select VernacularId "@ref"
				from plant_name.vernacularuse where vernacularuseid = vu1.vernacularuseid
				for xml path('VernacularName'), type),
	...
	(select --Taxon Properties
		...
		(select 
			...
			(select NameId "@nameRef",
				...
				from plant_name.Biostatus where BiostatusID = b1.BiostatusID
	...
	case when exists(select * from plant_name.annotation) then
		(select -- Annotations
			...
			(select NameId "@nameRef",
				ConceptID "@conceptRef"
				from plant_name.Annotation where AnnotationID = a1.AnnotationID
				for XML path('Taxon'), type),
			Annotation "AnnotationText",
			(select AccordingToId "@ref"
				from plant_name.Annotation where AnnotationID = a1.AnnotationID
				for XML path('AccordingTo'), type)
			from plant_name.annotation a1
			for xml path('Annotation'), type)
		for xml path('Annotations'), type)
	else '' end

  from plant_name.Metadata for xml Path ('DataSource'), type)
