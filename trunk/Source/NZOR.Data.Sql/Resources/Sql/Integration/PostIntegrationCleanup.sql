--remove any consensus names that have no provider names
declare @nids table(nameid uniqueidentifier, row int identity)

insert @nids
select distinct n.NameID
from consensus.Name n
left join provider.Name pn on pn.ConsensusNameID = n.NameID
where pn.NameID is null
union 
select distinct n.ConsensusNameID
from admin.BrokeredName bn
inner join provider.Name n on n.NameID = bn.NZORProviderNameId
where n.ConsensusNameID is not null

declare @pos int, @cnt int, @nameid uniqueidentifier
select @cnt = COUNT(*), @pos = 1 from @nids

while (@pos <= @cnt)
begin
	select @nameid = nameid from @nids where row = @pos
		...
		VALUES (newid(), 'consensus.Name', @nameId, null, getdate())

		INSERT INTO [dbo].[Deprecated]([DeprecatedId], [Table], [OldId], [NewId], [DeprecationDate])
		select newid(), 'consensus.Concept', ConceptId, null, getdate()
