IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'sprSelect_NamesWithAuthor')
	BEGIN
		DROP  Procedure  sprSelect_NamesWithAuthor
	END

GO

CREATE Procedure sprSelect_NamesWithAuthor
	@authorPk int
AS
	...
	where ' ' + nameauthorsbasionymauthors + ' ' like '% ' + cast(@authorpk as varchar(20)) + ' %' or
		' ' + nameauthorscombinationauthors + ' ' like '% ' + cast(@authorpk as varchar(20)) + ' %'
	

GO


GRANT EXEC ON sprSelect_NamesWithAuthor TO PUBLIC
