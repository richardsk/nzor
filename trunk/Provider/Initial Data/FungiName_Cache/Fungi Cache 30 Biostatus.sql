IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[FungiCache30]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[FungiCache30]
GO
...
CREATE procedure [dbo].[FungiCache30]
as


delete [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.fungi_name.Biostatus;
...
with bios as (
	  select
		...
		case b.BioStatusDescription 
			...
			when 'Exoctic Casual' then 'Exotic'
		...
		ROW_NUMBER() OVER(PARTITION BY n.NameGuid, geo.georegionname 
		 ...
		 inner join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].Name_Cache.fungi_name.Name na on nb.NameBiostatusNameFk
			= Na.NameId
...
from #bios b
inner join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].Name_Cache.fungi_name.taxonconcept tc on 
...
insert into [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.fungi_name.Biostatus(
...
grant execute on dbo.[FungiCache30] to dbi_user
