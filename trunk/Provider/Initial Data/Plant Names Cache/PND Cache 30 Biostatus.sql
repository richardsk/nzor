IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[PNDCache30]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[PNDCache30]
GO
...
CREATE procedure [dbo].[PNDCache30]
as

delete [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.plant_name.Biostatus;
...
with bios as (
	  select
		...
		ROW_NUMBER() OVER(PARTITION BY n.NameGuid, geo.georegionname 
		 ...
		 inner join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].Name_Cache.plant_name.Name na on nb.NameBiostatusNameFk
			= Na.NameId
...
from #bios b
inner join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].Name_Cache.plant_name.taxonconcept tc on 
...
insert into [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.plant_name.Biostatus(
...
grant execute on dbo.[PNDCache30] to dbi_user
