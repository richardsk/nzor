IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[NZACCache10]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[NZACCache10]
GO
...
CREATE procedure [dbo].[NZACCache10]
as

set concat_null_yields_null off

delete [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzac_name.[Name]

INSERT INTO [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzac_name.[Name](NameId, IsScientific,
...
from dbo.tblName n ) DS
	inner join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzac_name.Name na on DS.NameGuid = Na.NameId
...
from tblName n
inner join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzac_name.[Name] n2 on n2.nameid = n.NameGuid
...
--fix rank names
update [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzac_name.[Name]
set [rank] = 'subsection'
...
grant execute on dbo.[NZACCache10] to dbi_user
