IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[NZIBCache10]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[NZIBCache10]
GO
...
CREATE procedure [dbo].[NZIBCache10]
as

set concat_null_yields_null off

delete [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzib_name.[Name]

INSERT INTO [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzib_name.[Name](NameId, IsScientific,
...
select na.NameGuid, 1,
	...
	case when na.NameNomCode is null then 'NCGN' when na.NameNomCode = '' then 'NCGN' else na.NameNomCode end as NameNomCode, 
...
where na.NameSuppress = 0 and isnull(pref.namesuppress,0) = 0
	and (fn.FlatNameCanonical is null or fn.FlatNameCanonical <> 'kingdomtemp')
	and na.namefull <> 'root'
	and (na.NameNomCode is null or na.NameNomCode <> 'NCCN')
	and isnull(na.namemisapplied,0) = 0 and isnull(na.nameinvalid,0) = 0
...
from dbo.tblName n ) DS
	inner join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzib_name.Name na on DS.NameGuid = Na.NameId
...
from tblName n
inner join [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzib_name.[Name] n2 on n2.nameid = n.NameGuid


-- For any names that have a suppressed/misapp parent, we need to include the parent names (only the basic info)
INSERT INTO [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzib_name.[Name](NameId, IsScientific,
	createdDate, 
	modifiedDate, 
	NameFull, 	
...
where (pn.NameSuppress = 1 or pn.NameMisapplied = 1 or pn.nameinvalid = 1) and na.NameSuppress = 0 
	and isnull(na.NameMisapplied,0) = 0 and isnull(na.nameinvalid,0) = 0
	and (kfn.FlatNameCanonical is null or kfn.FlatNameCanonical <> 'kingdomtemp') 
	and na.NameNomCode is not null and na.NameNomCode <> 'NCCN' and na.NameNomCode <> ''


--fix rank names
update [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzib_name.[Name]
set [rank] = 'subsection'
where [rank] = 'subsection(ICZN)'

update [NAME_CACHE.DATABASES.LANDCARERESEARCH.CO.NZ].name_cache.nzib_name.[Name]
set [rank] = 'section'
where [rank] = 'section(ICBN)'

go

grant execute on dbo.[NZIBCache10] to dbi_user
